...
int[] buttonFrameOn = {150,25,100,50};
String buttonStringOn = "On";

int[] buttonFrameOff = {150,150,100,50};
String buttonStringOff = "Off";

int[] buttonFrame8t = {50,300,100,50};
String buttonString8t = "8 timer";

int[] buttonFrameTest = {250,300,100,50};
String buttonStringTest = "test";
...
void draw() {
  scaleGUI();
  drawButton(buttonFrameOn, buttonStringOn);
  drawButton(buttonFrameOff, buttonStringOff);
  drawButton(buttonFrame8t, buttonString8t);
  drawButton(buttonFrameTest, buttonStringTest);
}

void mousePressed() {
   if (mouseInside(buttonFrameOn)) {
      btWrite("1");
   } else
   if (mouseInside(buttonFrameOff)){
      btWrite("0");
   } else
   if (mouseInside(buttonFrame8t)){
      btWrite("8");
   } else
   if (mouseInside(buttonFrameTest)){
      btWrite("t");
   }
}